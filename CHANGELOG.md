...
- **详情页新增 Emby 风格全页氛围背景**（设置 → 界面 →「海报氛围背景」，默认开）：开启时去掉英雄区横幅，海报、标题、元数据与播放钮直接浮在整页 backdrop 氛围背景上——背景低档模糊（能认出是哪部剧）、固定不随滚动，全屏遮罩日间白雾 / 夜间黑纱，无额外压暗带；头部文字与按钮颜色跟随外观自适应（日间深字、夜间白字），白色按钮带柔和投影分界。关闭时恢复原有清晰横幅布局。条目没有 backdrop 图时始终走清晰横幅布局。氛围图为 800 宽小图 + 512px 解码下采样，不为糊图拉原画。
