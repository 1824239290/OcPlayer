...
- **iPhone 上点开 HUD 功能按钮（弹幕/字幕/音轨/章节/更多）后画面被拉伸、整个 HUD 卡死无响应（issue #5）**：两个症状同根。跳过按钮层为「面板打开时浮到面板上方」按面板**自然高度**（弹幕 8 行 ≈ 324pt）动态抬升底部 padding——该层 VStack 总高 ≈ 482pt，必然超出 iPhone 横屏 375–430pt 的屏幕；它是播放器 ZStack 的子层，溢出把 ZStack 撑大，视频 surface 的 frame 跟着变（实测高度在 318–402pt 间震荡），内核 resize + CoreAnimation 把旧 drawable 拉伸到新 bounds = 「画面缩放异常」；而「面板高度实测 → 回写 @State → 喂回跳过层/面板布局」又反向影响测量值，形成每帧震荡的布局反馈循环——测量回调打了三万多条日志且值各不相同，主线程被布局打满，触摸事件全部饿死 = 「点开面板整个播放器卡死」。斩环三刀：**① 面板打开时跳过层整层淡出**（只动 opacity、布局尺寸恒定——面板已占据该屏幕区域，动态让位本无意义）；**② `panelContentHeight` 跨视图 Binding 链（Screen→Overlay→簇→卡片四层）删除**，隐藏镜像的实测值改为卡片私有 state（测量→布局→测量的环收敛在卡片内部，初值 0→测得自然高→切滚动后值不再变）；**③ 面板内容高度上限按 HUD 实测可用高折算**（减簇底距/按钮行/间距/卡片内边距，下限 200pt），iPhone 横屏上弹幕面板收进 ~228pt 滚动列表、不再撑出屏幕，竖屏/macOS 窗口放得下时行为不变。顺带修复自检路径（启动即播）的 iOS 方向锁失效：方向回调装配从 WindowGroup.onAppear 提前到 `App.init`（首次 `presentedPlayer` 赋值早于 onAppear 时 `setPlayerActive` 从未被调用），scene 查找放宽到前台 scene 兜底任意 UIWindowScene，且 scene 尚未连接时把方向刷新攒下来、监听 `UIScene.didActivateNotification` 补发。验证：iOS 模拟器 + macOS 实测——面板展开期间视频 surface 零 resize、画面与关闭面板时逐像素一致、弹幕↔字幕切换即时响应、跳过层随面板开合淡入淡出；模拟器上「播放时自动横屏」受平台限制（`requestGeometryUpdate` 仅真机生效）无法覆盖，issue 原始场景请以真机为准。
