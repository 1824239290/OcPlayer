...
- **资源搜索筛选排序对齐 MoviePilot 网页端**：原先「站点选择 + 一颗筛选按钮弹整表单 + 排序菜单」换成官方同款筛选条——搜索范围、排序（字段 + 升降序）与七组筛选（站点 / 季 / 促销 / 编码 / 质量 / 分辨率 / 制作组）平铺一行，每组点开是候选值 chips 下拉（头部「全选 / 清除」，可多选；iPhone 紧凑宽度自动升级为半屏 sheet）；激活的分组按钮 tint 玻璃高亮并记数，下方以「站点:观众 ×」样式的已选 chip 汇总、可逐个移除或一键清除。全部走原生 Liquid Glass（`glassEffect` 胶囊），候选值随搜索结果聚合并记忆化（随批次落账，不随界面求值重算）。原整表单筛选弹窗移除。
...
- **氛围背景不铺侧栏，侧栏整列空玻璃（尤其下半截）**：页面的氛围背景挂在详情列 ScrollView 的背景上，而 macOS 26 的侧栏玻璃只对窗外取景——垫在 split view 或窗口容器底下的内容都透不出来（最小工程实测），侧栏于是永远是一整条死黑。现在有氛围图的页面出现时向 `AppModel.windowAmbience` 声明、离屏时撤回（MoviePilot 资源搜索页声明海报、详情页声明 backdrop，与页面氛围层同一开关），侧栏收到声明期间摘掉系统玻璃底、把同源模糊图直接垫进列内：`BackdropAmbienceView` 新增 `.sidebar` 遮罩档，只全高压暗不沉回页面底色，图一直透到列底，行文字保持可读；返回 / 切到无氛围页自动恢复系统玻璃原样，iPhone 紧凑布局无侧栏不受影响。
