...
- **iPhone 上点开 HUD 功能按钮（弹幕/字幕/音轨/章节/更多）后画面被拉伸、整个 HUD 卡死无响应（issue #5）**：两个症状同根。跳过按钮层为「面板打开时浮到面板上方」按面板**自然高度**（弹幕 8 行 ≈ 324pt）动态抬升底部 padding——该层 VStack 总高 ≈ 482pt，必然超出 iPhone 横屏 375–430pt 的屏幕；它是播放器 ZStack 的子层，溢出把 ZStack 撑大，视频 surface 的 frame 跟着变（实测高度在 318–402pt 间震荡），内核 resize + CoreAnimation 把旧 drawable 拉伸到新 bounds = 「画面缩放异常」；而「面板高度实测 → 回写 @State → 喂回跳过层/面板布局」又反向影响测量值，形成每帧震荡的布局反馈循环——测量回调打了三万多条日志且值各不相同，主线程被布局打满，触摸事件全部饿死 = 「点开面板整个播放器卡死」。斩环三刀：**① 面板打开时跳过层整层淡出**（只动 opacity、布局尺寸恒定——面板已占据该屏幕区域，动态让位本无意义）；**② `panelContentHeight` 跨视图 Binding 链（Screen→Overlay→簇→卡片四层）删除**，隐藏镜像的实测值改为卡片私有 state（测量→布局→测量的环收敛在卡片内部，初值 0→测得自然高→切滚动后值不再变）；**③ 面板内容高度上限只用「档位 + 静态值」推导**——iPhone 横屏（`verticalSizeClass == .compact`，屏高范围 375–440pt）按最保守的 375pt 推导出 213pt 上限，超限收进滚动列表；竖屏 / iPad / macOS 放得下时行为不变。⚠️ 面板高度**绝不能**依赖「实测 HUD 高度」推导：测量值回写 @State 再喂回面板布局，HUD/面板的高度变化又反过来改变测量值——横屏（cap 触发滚动的分支）实测每帧震荡 9000+ 条永不收敛，主线程被布局打满；任何「测量 → 回写布局」的用法在这个视图层级都是循环燃料。顺带修复自检路径（启动即播）与普通点播的 iOS 方向锁失效：**方向切换不再走「AppModel 持闭包 + App.init 装配」**——SwiftUI 会多次创建 App 值，`App.init` 经 `@State` 访问到的 appModel 与实际存储/注入 RootView 的不是同一实例（ObjectIdentifier 实测 `0x8000` vs `0xb200`），闭包装配静默丢失、播放中横屏锁整个失效；改为 AppModel 在 `presentedPlayer` didSet 广播 `playerPresentationDidChangeNotification`，AppDelegate 在自身 init（app 启动最早时机）监听并执行方向切换，scene 尚未连接时把刷新攒下来、监听 `UIScene.didActivateNotification` 补发。验证：iOS 模拟器 + macOS 实测——面板展开期间视频 surface 零 resize、画面与关闭面板时逐像素一致、弹幕↔字幕切换即时响应、跳过层随面板开合淡入淡出；启动即播与正常点播均自动进横屏（模拟器实测 `requestGeometryUpdate` 生效，此前误判为平台限制）。
