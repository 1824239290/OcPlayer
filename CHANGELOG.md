...
- **详情页新增 Emby 风格全页氛围背景**：条目的 backdrop 铺满整页、高斯模糊后垫在内容底下，固定不随滚动；夜间压暗、日间白雾化（文字对比度由遮罩保证），底部渐隐回页面底色。顶部横幅与氛围层的衔接改为渐隐融接（横幅渐隐调透），不再有实色横带接缝。氛围图为 800 宽小图 + 512px 解码下采样，不为糊图拉原画。
- **首页背景改为库内随机 backdrop 渐变轮播**：服务器端 `SortBy=Random` 从全库随机取 8 张带 backdrop 的电影 / 剧集（查询失败或为空回退首页已加载条目），同样模糊+雾化垫底，每 12 秒淡入淡出换一张（减弱动态效果时保持静态首图）。首图进缓存后才亮相，其余后台预热，切换零占位闪烁；切换服务器会话自动重取。
