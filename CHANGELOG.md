...
- **详情页新增 Emby 风格全页氛围背景**（设置 → 界面 →「海报氛围背景」，默认开）：开启时去掉英雄区横幅，海报、标题、元数据与播放钮直接浮在整页 backdrop 氛围背景上——背景低档模糊（能认出是哪部剧）、固定不随滚动，顶部压暗保白字头部可读（日间主题同样如此，下半段沉回页面底色）；关闭时恢复原有清晰横幅布局。条目没有 backdrop 图时始终走清晰横幅布局。氛围图为 800 宽小图 + 512px 解码下采样，不为糊图拉原画。
