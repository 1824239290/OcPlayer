...
# v0.1.9+dolby.1 暂未附 iOS 资产（fork 仅 macOS arm64 手工包）。
# iOS 打包继续钉在最近一次带全平台产物的 tag；等 fork 补齐 iOS 后再一起升。
ERIKA_VERSION="${ERIKA_VERSION:-v0.1.7+dolby.3}"
...
if [[ "$IS_LOCAL_BUILD" -eq 1 ]]; then
    ...
    ARTIFACT_BASE="$APP_NAME-$SAFE_TAG-b$BUILD_NUMBER-$SAFE_BUILD-ios-unsigned"
else
